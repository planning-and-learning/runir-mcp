...
The emitted renderings are selected by `dump_result(..., formats=...)`. The conventions below describe the `.psv` rendering — the `.md` and `.json` files are mechanical renders of the same logical tables (see [The same tables in Markdown and JSON](#the-same-tables-in-markdown-and-json)).
