...
def reformat_policy(options: ReformatPolicyOptions) -> ReformatPolicyResult:
    ...
    description = options.sketch_file.read_text(encoding="utf-8")
    parsed = parse_policy_description(context, description)
    options.sketch_file.write_text(f"{parsed}\n", encoding="utf-8")
    return ReformatPolicyResult(sketch_file=options.sketch_file, kind="sketch")
...
def create_empty_policy(options: CreateEmptyPolicyOptions) -> ReformatPolicyResult:
    ...
    options.sketch_file.parent.mkdir(parents=True, exist_ok=True)
    options.sketch_file.write_text(f"{parsed}\n", encoding="utf-8")
    return ReformatPolicyResult(sketch_file=options.sketch_file, kind="sketch")
